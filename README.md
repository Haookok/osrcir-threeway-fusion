...
This repo does not store API keys in source control. Copy `.env.example` or export the following variables before running API-based scripts:

```sh
export DASHSCOPE_API_KEY=your_dashscope_key
export MINIMAX_API_KEY=your_minimax_key
export GPT4O_API_KEY=your_gpt4o_proxy_key
export GPT4O_API_BASE=https://api.zhizengzeng.com/v1
export GPT4O_MODEL=gpt-4o-2024-08-06
```

The current codebase uses:

- `DASHSCOPE_API_KEY` for `src/cloudgpt_api.py`
- `MINIMAX_API_KEY` for proxy image generation
- `GPT4O_API_KEY` for `scripts/gpt4o_fiq_comparison.py`
