...
def fiq(
    ...
    if analysis_output_path:
        ...
        with io.open(analysis_output_path, 'w', encoding='utf-8') as f:
            for sample_idx in range(len(labels)):
                true_idx = torch.where(labels[sample_idx] == True)
                rank = true_idx[0].item() + 1
                easy_sample_flag = bool(torch.sum(labels[sample_idx][:50]) == 1)
                payload = {
                    "target_name": target_names[sample_idx].split("/")[-1],
                    "top_predictions": [sample.split("/")[-1] for sample in sorted_index_names[sample_idx][:100]],
                    "rank": rank,
                    "hit@50": easy_sample_flag,
                }
    ...
    if analysis_output_path:
        ...
        with io.open(analysis_output_path, 'w', encoding='utf-8') as f:
            for sample_idx in range(len(labels)):
                ...
                rank = true_idx[0].item() + 1
                payload = {
                    "reference_name": reference_names[sample_idx],
                    "target_name": target_names[sample_idx],
                    "top_predictions": [sample.split("/")[-1] for sample in sorted_index_names[sample_idx][:100]],
                    "rank": rank,
                    "hit@10": bool(torch.sum(labels[sample_idx][:10]) == 1),
                }
                f.write(json.dumps(payload, ensure_ascii=False) + "\n")
    assert torch.equal(torch.sum(labels, dim=-1).int(), torch.ones(len(target_names)).int())
