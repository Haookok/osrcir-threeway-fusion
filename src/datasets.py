...
# ==================== 兼容性修改开始 ====================
# 这个辅助函数是本次为 FashionIQ 本地图片扩展名不一致而新增的。
# 原始代码默认所有图片都是 .png，但你本地很多图片实际是 .jpg / .jpeg。
# 因此这里统一做一次“自动尝试多个后缀”的解析：
# 1. 如果传进来的路径本来就存在，直接返回
# 2. 如果不存在，就在同名基础上依次尝试 .png / .jpg / .jpeg
# 3. 如果都找不到，则返回原路径，让后续代码按原逻辑报错，便于定位缺图问题
# 这样做的目的不是隐藏错误，而是先优先解决“明明有图，但后缀不一致”的情况。
def resolve_local_image_path(image_path: Union[Path, str]) -> Path:
    ...
    for suffix in suffix_candidates:
        resolved_path = candidate_path.with_suffix(suffix)
        ...
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val']:
                    # ==================== 兼容性修改开始 ====================
                    # 原项目这里写死为 .png。
                    # 现在改为先构造默认路径，再通过 resolve_local_image_path 自动匹配真实文件后缀。
                    reference_image_path = str(resolve_local_image_path(self.dataset_path / 'images' / f"{reference_name}.png"))
                    target_name = self.triplets[index]['target']
                    target_image_path = str(resolve_local_image_path(self.dataset_path / 'images' / f"{target_name}.png"))
                    # ==================== 兼容性修改结束 ====================
                    '''
                    ...
                    # ==================== 兼容性修改开始 ====================
                    # 测试集同样不再假设图片一定是 .png，而是自动匹配本地真实后缀。
                    reference_image_path = str(resolve_local_image_path(self.dataset_path / 'images' / f"{reference_name}.png"))
                    # ==================== 兼容性修改结束 ====================
                    '''
                        For GPT4V api, the preprocess are included in the model. Therefore, only needs the image urls (i.e., image paths)
                ...
                # ==================== 兼容性修改开始 ====================
                # classic 模式是后半段检索阶段真正遍历整个图库时使用的。
                # 这里如果仍然写死 .png，就会在你本地 jpg 图库上直接报 FileNotFoundError。
                image_path = str(resolve_local_image_path(self.dataset_path / 'images' / f"{image_name}.png"))
                # ==================== 兼容性修改结束 ====================
                '''
                    For GPT4V api, the preprocess are included in the model. Therefore, only needs the image urls (i.e., image paths)
                '''
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            if self.split == 'val':
                ...
                # Pad ground truth image IDs to fixed length for collate_fn
...
class COCODataset(Dataset):
    ...
    def load_blip_sample(self, sample):
        ...
        fpath = str(os.path.join(self.root_dir, f'{int(val_img_id):012d}.jpg'))
...
class VAWValSubset(VAWDataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(f"[WARN] Skipping VAW sample {index}: {e}")
            return None
